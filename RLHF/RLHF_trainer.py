...
class RLHFTrainer(Trainer):
    def __init__(self, args):
        ...
        self.train_batch = 0
        self.update_step = 0
        self.start_step = self.actor_critic.load_parameters(self.args.RLHF_load)
    ...
    def RLHF_train(self):
            ...
            for epoch in range(self.args.epoch):
                for idx, (
                    ...
                    with self.accelerator.accumulate(warp_actor_critic):
                        self.update_step += 1
                        sequence_logit = warp_actor_critic.forward('actor', _sequence, attention_mask=_sequence != 0).logits
                        ...
                        action_value = warp_actor_critic.forward('critic', _sequence, attention_mask=_sequence != 0)
                        ...
                        self.accelerator.backward(actor_loss + self.args.vf_coef*critic_losses)

                        if self.accelerator.sync_gradients and self.args.clip_grad_norm > 0:
                            self.accelerator.clip_grad_norm_(RLHF_optim.param_groups[0]['params'], self.args.clip_grad_norm)
                        RLHF_optim.step()
                        RLHF_lr_scheduler.step()
                        RLHF_optim.zero_grad()

                    approx_kl = 0.5 * masked_mean(log_ratio**2, _action_mask).float().item()
                    policy_kl = masked_mean(-log_ratio, _action_mask).float().item()
                    critic_clip_frac = masked_mean(torch.gt(critic_losses2, critic_losses1).float(), _action_mask).float().item()
                    action_clip_frac = masked_mean(torch.gt(action_losses2, action_losses1).float(), _action_mask).float().item()
                    total_losses = (action_losses + self.args.vf_coef * critic_losses - self.args.entropy_weight * entropy_losses).float().item()
                    log_metrics = torch.tensor([action_losses, critic_losses, entropy_losses, total_losses,
                                                approx_kl, policy_kl, action_clip_frac, critic_clip_frac],
                                               device=self.accelerator.device)
                    log_metrics = self.accelerator.reduce(log_metrics, reduction='mean')  # [8]
                    if self.accelerator.is_main_process:
                        writer.add_scalar('training/actor_loss_mean', float(log_metrics[0]), self.update_step)
                        writer.add_scalar('training/critic_loss_mean', float(log_metrics[1]), self.update_step)
                        writer.add_scalar('training/entropy_loss_mean', float(log_metrics[2]), self.update_step)
                        writer.add_scalar('training/total_loss_mean', float(log_metrics[3]), self.update_step)
                        writer.add_scalar('training/approx_kl', float(log_metrics[4]), self.update_step)
                        writer.add_scalar('training/policy_kl', float(log_metrics[5]), self.update_step)
                        writer.add_scalar('training/action_clip_frac', float(log_metrics[6]), self.update_step)
                        writer.add_scalar('training/critic_clip_frac', float(log_metrics[7]), self.update_step)

                        RLHF_lr = RLHF_lr_scheduler.get_lr()[0]
                        print(f"Batch: {self.train_batch} | Epoch: {epoch} | Update_step: {self.update_step}"
                              f" | RLHF_lr: {RLHF_lr:.7f} | policy_kl: {log_metrics[5]:.6f}")
                    if log_metrics[5] > self.args.policy_kl_threshold:
                        early_stop = True
                    if self.accelerator.sync_gradients and early_stop:
                        break
                if early_stop:
                    break
        ...
        warp_actor_critic, RLHF_optim, train_loader, val_loader, RLHF_lr_scheduler = self.accelerator.prepare(
            self.actor_critic, RLHF_optim, train_loader, val_loader, RLHF_lr_scheduler
        )
        ...
        for eps in range(self.args.num_episodes):
            for step_i, batch in enumerate(train_loader):
                ...
                with torch.no_grad():
                    ...
                    reward_data = [
                        train_data.get_list_reward_hard_encode(
                    ...
                    log_d = torch.tensor([total_count, total_reward_sum, total_non_exist_rate, total_repeat_rate, total_correct_count],
                                         device=self.accelerator.device)
                    log_d = self.accelerator.reduce(log_d)  # [5]
                    if self.accelerator.is_main_process:
                        writer.add_scalar('sampling/total_reward', log_d[1]/log_d[0], log_d[0]//sample_num)
                        writer.add_scalar('sampling/total_non_exist_rate', log_d[2]/log_d[0], log_d[0]//sample_num)
                        writer.add_scalar('sampling/total_repeat_rate', log_d[3]/log_d[0], log_d[0]//sample_num)
                        writer.add_scalar('sampling/total_correct_count', log_d[4]/log_d[0], log_d[0]//sample_num)
                    if self.args.lr > 0:
                        ...
                        if self.args.reward_scale:
                            list_rewards_gather = self.accelerator.gather(list_rewards)
                            running['list'].update(list_rewards_gather)
                        ...
                        for _, __ in enumerate(complete_data['input_ids']):
                            assert total_rewards[i][__ == self.tokenizer.eos_token_id].shape[0]-1 == 1
                            total_rewards[_][__ == self.tokenizer.eos_token_id][-1] += list_rewards[_]
                        # total_rewards[complete_data['input_ids'] == self.tokenizer.eos_token_id] += list_rewards
                        ...
                        if self.args.fine_grain_reward:
                            ...
                            for i in range(bs * sample_num):
                                ...
                                assert total_rewards[i][complete_data['input_ids'][i] == 13].shape[0]-4 == item_rewards[i].shape[0]
                                total_rewards[i][complete_data['input_ids'][i] == 13][4:] += item_rewards[i]
                                # total_rewards[i][complete_data['input_ids'][i] == 13] += item_rewards[i]
                    ...
                    if self.accelerator.is_main_process:
                        print(f'rlhf learning in eps_{eps} | step_{step_i} | example: {len(memories)} | max_length: {complete_data["input_ids"].shape[1]}')
